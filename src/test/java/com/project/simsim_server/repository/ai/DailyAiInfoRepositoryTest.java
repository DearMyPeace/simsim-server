...
//        // given
//        DailyAiInfo data
//                = dailyAiInfoRepository.findById(1L).orElseThrow(() -> new RuntimeException());
//
//        // when
//        String summary = null;
//        String reply = null;
//        if (isBase642(data.getDiarySummary())) {
//
//        }
//    }
    ...
    private boolean isBase642(String data) {
        try {
            byte[] decodedBytes = Base64.getDecoder().decode(data);
            String decodedString = new String(decodedBytes, StandardCharsets.UTF_8);

            // 추가적인 유효성 검사: UTF-8 인코딩 확인
            if (!isValidUTF8(decodedString)) {
                return false;
            }

            // 추가적인 유효성 검사: 특정 길이 확인
            if (decodedString.length() < 10) {
                return false;
            }

            // 추가적인 유효성 검사: 특정 패턴 확인 (예: 특정 키워드 포함 여부)
            if (!decodedString.contains("감사합니다")) {
                return false;
            }
